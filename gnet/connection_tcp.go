...
func NewTcpConnector(config *ConnectionConfig, address string, codec Codec, handler ConnectionHandler, isFixedHead bool) *TcpConnection {
	...
	newConnection := createTcpConnection(config, address, codec, handler)
...
func createTcpConnection(config *ConnectionConfig, address string, codec Codec, handler ConnectionHandler) *TcpConnection {
...
// 连接
func (_self *TcpConnection) Connect(address string) bool {
	conn, err := net.DialTimeout("tcp", address, time.Second)
	if err != nil {
		_self.isConnected = false
		// logger.Error("Connect failed %v: %v", _self.GetConnectionId(), err.Error())
		// if _self.handler != nil {
		// 	_self.handler.OnConnected(_self, false)
		// }
		return false
	}
	_self.conn = conn
...
// 开启读写协程
func (_self *TcpConnection) Start(ctx context.Context, netMgrWg *sync.WaitGroup, onClose func(connection Connection)) {
...
func (_self *TcpConnection) readLoopString() {
	defer func() {
		if err := recover(); err != nil {
...
// 收包过程
func (_self *TcpConnection) readLoop() {
	defer func() {
		if err := recover(); err != nil {
			logger.Error("readLoop fatal %v: %v", _self.GetConnectionId(), err.(error))
			LogStack()
...
// 发包过程
func (_self *TcpConnection) writeLoop(ctx context.Context) {
	defer func() {
		if err := recover(); err != nil {
...
// 关闭
func (_self *TcpConnection) Close() {
...
func (_self *TcpConnection) Send(command PacketCommand, message proto.Message) bool {
...
func (_self *TcpConnection) SendPacket(packet Packet) bool {
	if !_self.isConnected {
		return false
	}
	// NOTE:当sendPacketCache满时,这里会阻塞
...
func (_self *TcpConnection) TrySendPacket(packet Packet, timeout time.Duration) bool {
	if timeout == 0 {
		// 非阻塞方式写chan
		select {
...
// 创建用于批量发包的RingBuffer
func (_self *TcpConnection) createSendBuffer() *RingBuffer {
...
// 创建用于批量收包的RingBuffer
func (_self *TcpConnection) createRecvBuffer() *RingBuffer {
...
// 发包RingBuffer
func (_self *TcpConnection) GetSendBuffer() *RingBuffer {
	return _self.sendBuffer
}

// 收包RingBuffer
func (_self *TcpConnection) GetRecvBuffer() *RingBuffer {
	return _self.recvBuffer
}

// LocalAddr returns the local network address.
func (_self *TcpConnection) LocalAddr() net.Addr {
	if _self.conn == nil {
		return nil
	}
...
func (_self *TcpConnection) RemoteAddr() string {
...
func (_self *TcpConnection) GetSendPacketChanLen() int {
